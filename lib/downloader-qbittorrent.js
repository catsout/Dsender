...
class QBittorrent extends DownloaderBase {
     ...
     * @returns 
     */
    addP2pTask(taskparams) {
        const p2pParams = taskparams.p2pParams;
        const formData = new FormData();
        formData.append('urls', p2pParams.magnetUrl);
        ...
        // p2p
        if(typeof(p2pParams.sequential) === 'boolean')
            formData.append('sequentialDownload', p2pParams.sequential.toString())
        if(typeof(p2pParams.firstLastPiece) === 'boolean')
            formData.append('firstLastPiecePrio', p2pParams.firstLastPiece.toString())

        const {hash, name} = DownloaderBase.getMagnetInfo(p2pParams.magnetUrl);
