...
export class WidgetTaskItem extends HTMLElement {
  ...
  set status(value) {
    ...
    this.speed_up.classList.toggle("hidden", !Boolean(value.btStatus));
    if(value.btStatus)
      this.speed_up.value = value.btStatus.uploadSpeed;

    this.speed_down.classList.toggle("hidden", value.stats !== ETaskStats.downloading);
    ...
    if(value.totalLength) {
      ...
      if(value.status === ETaskStats.downloading) {
