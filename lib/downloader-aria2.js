import {DownloaderStatus, Task, TaskParams, ETaskStats, TaskStatus, DownloaderConfig, EDownloaderType, EDownloaderStats, basename, TaskItem} from '../common.js';
...
class Aria2 extends DownloaderBase {
     ...
     */
    addTask(taskparams) {
        const urlParmas = taskparams.urlParmas;
        const params = {};
        ...
        // url
        if(typeof(urlParmas.threads) === 'number') {
            params.split = urlParmas.threads.toString();
            params['max-connection-per-server'] = urlParmas.threads.toString();
        }
        if(typeof(urlParmas.minsplit) === 'number') {
            params['min-split-size'] = urlParmas.minsplit.toString() + 'M';
        }
        if(urlParmas.header) {
            params.header = [];
            Object.entries(urlParmas.header).forEach(([k, v]) => {
                if(Boolean(v))
        ...
        return sendJsonrpc(this.getUrl(),
            'aria2.addUri',
            this.preParams([[urlParmas.url], params])
        ).then((result) => {
